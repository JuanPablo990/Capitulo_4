...
def ParticulaPosicion(ket,X):
    return detectarParticula(ket,X)
def transitarVectorVector(ket,ket2):

    return transitarVector(ket,ket2)
def valorEsperado(obs,ket):
    obsSobreket = lc.accion(obs, ket)
    bra = vc.conjugado_Matriz_Vector(obsSobreket)
    ket1 = vc.transpuesta_Matriz_Vector([ket])
    bra1 = vc.transpuesta_Matriz_Vector(bra)
    car = vc.multiplicacionMatrizMatriz(bra1, ket1)[0][0]
    return car
...
def varianzaObservable(obs,ket):
    nve = vc.multiplicacion(valorEsperado(obs, ket), (-1, 0))
    mve = vc.multiplicacionEscalarMatriz(lc.matrizIdentidad(len(obs)), nve)
    delta = vc.sumaMatrices(obs, mve)
    deltaCuadrado = vc.multiplicacionMatrizMatriz(delta, delta)
